...
{/* Mobile Menu */}
{isMenuOpen && (
  <div className="absolute top-full left-0 w-full bg-slate-300 shadow-lg lg:hidden">
    <ul className="flex flex-col gap-3 p-4 text-center text-slate-800">
      {userToken && (
        <>
          <li>
          ...
          <li>
            <NavLink to="/cart" className="relative  hover:text-green-800" onClick={() => setIsMenuOpen(false)}>
              Cart
              {numCart > 0 && (
                <div className="absolute -top-4 -right-4 flex size-5 items-center justify-center rounded-full bg-emerald-600 p-1 text-white">
                  {numCart}
                </div>
              )}
            </NavLink>
          ...
          <li>
            <NavLink to="/wishlist" className="relative  hover:text-green-800" onClick={() => setIsMenuOpen(false)}>
              Wishlist
              {numWishList > 0 && (
                <div className="absolute -top-4 -right-4 flex size-5 items-center justify-center rounded-full bg-emerald-600 p-1 text-white">
                  {numWishList}
